# -*- coding: utf-8 -*-
"""修正 notes 行序(预设已有 note 行 -> 走保序路径)并验证"""
...
req = urllib.request.Request(BASE + '/api/preset/notes',
                             data=json.dumps({'id': PID, 'notes': notes}).encode('utf-8'),
                             headers={'Content-Type': 'application/json'}, method='PUT')
r = json.loads(urllib.request.urlopen(req, timeout=30).read().decode('utf-8'))
print('notes ok =', r.get('ok'))
print()
print('--- 回读校验 ---')
d = json.loads(urllib.request.urlopen(BASE + '/api/preset/config?id=' + urllib.parse.quote(PID), timeout=20).read().decode('utf-8')) if False else None
line = open(r'F:\MIMO-Space\PEQ-WebUI\apo-presets\dt900-pro-x-mv1拟合.txt', encoding='utf-8').read().splitlines()
for l in line[:20]:
    print(l)
